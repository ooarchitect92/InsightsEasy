...
test('browser completes all five synthetic workflow screens',async({page})=>{
 ...
 async function connection(name:string,provider:string){await page.getByLabel('Connection name').fill(name);await page.getByLabel('Capability',{exact:true}).selectOption(provider);await page.getByRole('button',{name:'Create connection',exact:true}).click();await expect(page.getByText(name,{exact:true}).first()).toBeVisible();}
 await connection('Browser CRM','simulator_crm');
 await page.getByLabel('Connection name').fill('Browser Source');await page.getByLabel('Capability',{exact:true}).selectOption('signed_webhook');await page.getByLabel('Automatic CRM destination').selectOption({label:'Browser CRM'});await page.getByRole('button',{name:'Create connection',exact:true}).click();await expect(page.getByText('Browser Source',{exact:true}).first()).toBeVisible();await page.getByRole('button',{name:'Hide secret'}).click();
 await connection('Browser Website','web_collector');await connection('Browser Ads','simulator_ads');
 await page.getByRole('button',{name:/02Acquisition journeys|02 Acquisition journeys/}).click().catch(()=>page.getByRole('button',{name:/Acquisition journeys/}).click());
